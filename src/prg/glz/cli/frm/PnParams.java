...
public class PnParams extends JPanel {
    ...
    private void init() {
        JLabel lb;
        int nPosY = TOP;
        this.setBackground( new Color( 127,160,206 ) );
        // Layout Absoluto
        this.setLayout( null );
        {
            // Servidor
    ...
    private void ejecSyncCommit() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog( PnParams.frmPrincipal, "Problemas al traer formularios desde el Servidor WEB\n" + e.getMessage() );
        } finally {
    ...
    private void ejecCorre() {
        ...
        // Corre sincronizador de archivos. Registra el directorio y comienza el proceso
        Path dir = Paths.get( txDir.getText() );
        try {
            this.watchDir = new WatchDir( dir, frmWork );

            // La clase WatchDir es Runnable para poder utilizarla dentro de un Hilo (ver método run de WatchDir)
            new Thread( this.watchDir ).start();
        } catch (IOException e) {
            JOptionPane.showMessageDialog( PnParams.frmPrincipal, "No se pudo arrancar el servicio de sincronización\n" + e.getMessage() );
            return;
        }
    }
